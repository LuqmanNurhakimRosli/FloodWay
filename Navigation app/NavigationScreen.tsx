// Navigation Screen - GPS-like turn-by-turn navigation using OSRM road routing
// Legacy screen component (ShelterPage/NavigationPage are the primary views)
...
export function NavigationScreen({ route, onBack, onArrived }: NavigationScreenProps) {
    ...
    // Direction icon based on instruction text
    const getDirectionEmoji = (instruction: string) => {
        const instr = instruction.toLowerCase();
        if (instr.includes('left')) return '⬅️';
        if (instr.includes('right')) return '➡️';
        if (instr.includes('straight') || instr.includes('continue')) return '⬆️';
        if (instr.includes('u-turn')) return '↩️';
        if (instr.includes('roundabout')) return '🔄';
        if (instr.includes('arrive')) return '📍';
        if (instr.includes('north')) return '⬆️';
        if (instr.includes('south')) return '⬇️';
        if (instr.includes('east')) return '➡️';
        if (instr.includes('west')) return '⬅️';
        return '📍';
    };
        ...
        <div className="min-h-screen bg-gray-900 flex flex-col">
            ...
            <div className="bg-gray-800 p-4 flex items-center justify-between border-b border-gray-700">
                ...
                <div className="text-xs text-emerald-400">
                    {modeLabel[route.transportMode || 'car']}
                </div>
            </div>
                        ...
                        <>
                            ...
                            <div className="flex items-start gap-4 mb-4">
                                <div className="w-16 h-16 bg-emerald-500 rounded-2xl flex items-center justify-center text-3xl">
                                    {getDirectionEmoji(currentInstruction.instruction)}
                                </div>
